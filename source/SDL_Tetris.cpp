...
class SDLsubsystems{
	...
	void InitImage(){
		...
		/*
		std::vector<std::unique_ptr<imageloader>> vec;
		std::vector<std::unique_ptr<ApplyImage>> vecA;

		vec.push_back(std::unique_ptr<imageloader>(new imageloader()));
		for (int i = 0; i < vec.size(); i++)
			vec[i]->IMGLOAD(PATH, PNG);
		vecA.push_back(std::unique_ptr<ApplyImage>(new ApplyImage()));
		for (int i = 0; i < vecA.size(); i++)
			vecA[i]->APPIMG(cRender, cTexture, PNG);
		
		log("->> Vectors initialized successfuly\n\n");
		*/
	}
